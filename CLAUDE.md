...
- **`/usr/local/bin/bluetooth-reconnect.sh`** — single interactive `bluetoothctl` session (mirrors manual workflow); silent exit if already connected or if manual-mode flag exists; if paired tries connect up to 3×; if not paired does full scan→pair→trust→connect flow
- **`/usr/local/bin/bluetooth-control.sh`** — called by HA via SSH; accepts `pair` or `unpair` argument; `unpair` creates the manual-mode flag at `/var/lib/bluetooth-reconnect/manual-mode` and disconnects/removes the amp; `pair` removes the flag and triggers `bluetooth-reconnect.service --no-block`
...
`deploy.sh` generates `ha-snippets/<divisao>.yaml` after every deploy — paste its content into the Lovelace YAML editor for the room card. `ha-snippets/configuration.yaml` contains the shared `shell_command` entries (add once to HA `configuration.yaml`).

### Shell commands (shared, add once to configuration.yaml)
...
### Per-room dashboard card (generated by deploy.sh)
...
- `/var/lib/bluetooth-reconnect/manual-mode` — created by `unpair`, deleted by `pair`
